<template>
  <div>
    <h2 id="page-heading" data-cy="CountryHeading">
      <span v-text="$t('pruebaMonoliticaApp.country.home.title')" id="country-heading">Countries</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('pruebaMonoliticaApp.country.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CountryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('pruebaMonoliticaApp.country.home.createLabel')"> Create a new Country </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && countries && countries.length === 0">
      <span v-text="$t('pruebaMonoliticaApp.country.home.notFound')">No countries found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="countries">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('pruebaMonoliticaApp.country.countryName')">Country Name</span></th>
            <th scope="row"><span v-text="$t('pruebaMonoliticaApp.country.region')">Region</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="pruebaMonoliticaApp.country.delete.question" data-cy="countryDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-country-heading" v-text="$t('pruebaMonoliticaApp.country.delete.question', { id: removeId })">
