<template>
  <div>
    <h2 id="page-heading" data-cy="JobHistoryHeading">
      <span v-text="$t('pruebaMonoliticaApp.jobHistory.home.title')" id="job-history-heading">Job Histories</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('pruebaMonoliticaApp.jobHistory.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'JobHistoryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('pruebaMonoliticaApp.jobHistory.home.createLabel')"> Create a new Job History </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && jobHistories && jobHistories.length === 0">
      <span v-text="$t('pruebaMonoliticaApp.jobHistory.home.notFound')">No jobHistories found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="jobHistories">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('startDate')">
              <span v-text="$t('pruebaMonoliticaApp.jobHistory.startDate')">Start Date</span>
            ...
            <th scope="row" v-on:click="changeOrder('endDate')">
              <span v-text="$t('pruebaMonoliticaApp.jobHistory.endDate')">End Date</span>
            ...
            <th scope="row" v-on:click="changeOrder('language')">
              <span v-text="$t('pruebaMonoliticaApp.jobHistory.language')">Language</span>
            ...
            <th scope="row" v-on:click="changeOrder('job.id')">
              <span v-text="$t('pruebaMonoliticaApp.jobHistory.job')">Job</span>
            ...
            <th scope="row" v-on:click="changeOrder('department.id')">
              <span v-text="$t('pruebaMonoliticaApp.jobHistory.department')">Department</span>
            ...
            <th scope="row" v-on:click="changeOrder('employee.id')">
              <span v-text="$t('pruebaMonoliticaApp.jobHistory.employee')">Employee</span>
        ...
        <tbody>
          <tr v-for="jobHistory in jobHistories" :key="jobHistory.id" data-cy="entityTable">
            ...
            <td v-text="$t('pruebaMonoliticaApp.Language.' + jobHistory.language)">{{ jobHistory.language }}</td>
            <td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="pruebaMonoliticaApp.jobHistory.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-jobHistory-heading" v-text="$t('pruebaMonoliticaApp.jobHistory.delete.question', { id: removeId })">
